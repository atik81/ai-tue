...
if __name__ == "__main__":
    speak('Enter T for typing or S for speech recognition:')

use_speech_recognition = False
while True:
    try:
        input_method = input("Enter T for typing or S for speech recognition:")
        if input_method.lower() == "t":
            start_city = input("Enter start city: ")
            target_city = input("Enter target city: ")
        elif input_method.lower() == "s":
            speak('Enter your start city please')
            with sr.Microphone() as mic:
                recognizer.adjust_for_ambient_noise(mic, duration=0.2)
                audio = recognizer.listen(mic)
                start_city = recognizer.recognize_google(audio)
            ...
            with sr.Microphone() as mic:
                recognizer.adjust_for_ambient_noise(mic, duration=0.2)
                audio = recognizer.listen(mic)
                target_city = recognizer.recognize_google(audio)
                target_city = target_city.title()
                print(f"Target city recognized: {target_city}")
                engine.say(f"Target city recognized: {target_city}")
                engine.runAndWait()
        else:
            continue

        use_speech_recognition = not use_speech_recognition
        break
...
while start_city not in distances or target_city not in distances:
        ...
        if start_city not in distances:
            raise ValueError("Invalid starting city")
